...
struct SetupView: View {
    ...
    private var textAndColorSection: some View {
        VStack(spacing: 12) {
            signalTextField
            colorSwatchPicker
        }
    ...
    private var signalTextField: some View {
        ...
        VStack(spacing: 8) {
            ...
            HStack(spacing: 14) {
                ForEach(UserSettings.signalColors, id: \.id) { entry in
                    colorSwatchButton(entry: entry)
                }
            }
        }
    }

    private func colorSwatchButton(entry: (id: String, color: Color)) -> some View {
